...
var jquerycontextmenu={
	...
	init:function($, $target, $contextmenu){
		...
		if ($.inArray($contextmenu.get(0).id, this.builtcontextmenuids)==-1) //if this context menu hasn't been built yet
		...
		$target.bind("contextmenu", function(e){
			...
			// Alex Yarmula:: modification: To know which element the callback belongs to
			$contextmenu.find("a.context_menu_link").each(function() {
							this.service_name = $target.text();
			});
			return false;
		})
	}
